    ...
    <section class="bench" id="bench" aria-labelledby="bench-title">
      ...
      <div class="bench-grid">
        <section class="recipe-sheet" aria-labelledby="recipe-title">
          <div class="sheet-heading"><div><span class="step-number">01</span><h3 id="recipe-title">Wrapper recipe</h3></div><span id="version-stamp" class="stamp">Unsaved</span></div>
          <div class="saved-row">
            <label for="saved-recipes">Saved recipe</label>
            <select id="saved-recipes"><option value="">New recipe</option></select>
            <button class="text-button" id="new-recipe" type="button">Start fresh</button>
          </div>
        ...
        <section class="queue-sheet" aria-labelledby="queue-title">
          ...
          <div class="queue-empty" id="queue-empty"><span aria-hidden="true">≋</span><p>Your numbered job tickets will appear here.</p></div>
          <ol class="queue-list" id="queue-list" aria-label="Voice tracks"></ol>
          <div class="render-panel">
            <div class="render-summary"><strong id="render-summary">Ready when your queue is.</strong><span id="tier-note">Free batches include up to 3 tracks.</span></div>
...
function freshRecipe(): Recipe {
  const timestamp = now();
  return { id: uid(), version: 0, name: 'My show', targetLufs: -16, bedDb: -24, naming: '{recipe}-{number}-{source}', startNumber: 1, createdAt: timestamp, updatedAt: timestamp };
}
...
function renderQueue(): void {
  ...
  list.innerHTML = jobs.map((job, index) => {
    const state = job.error ? `<span class="job-state error">Error · ${escapeHtml(job.error)}</span>` : job.output
      ? `<span class="job-state success">Wrapped · ${job.output.durationSeconds.toFixed(1)} s</span>`
      : '<span class="job-state">Waiting</span>';
    const output = job.output && job.url ? `<div class="job-output"><audio controls preload="metadata" src="${job.url}" aria-label="Preview ${escapeHtml(job.output.name)}"></audio><a class="button quiet" download="${escapeHtml(job.output.name)}" href="${job.url}">Download WAV</a></div>` : '';
    return `<li class="job-ticket ${job.output ? 'complete' : ''}"><div class="ticket-number">${String(index + 1).padStart(2, '0')}</div><div class="job-copy"><strong>${escapeHtml(job.file.name)}</strong><span>${(job.file.size / 1024 / 1024).toFixed(1)} MB → ${escapeHtml(outputName($<HTMLInputElement>('#naming-template').value, $<HTMLInputElement>('#recipe-name').value, job.file.name, Number($<HTMLInputElement>('#start-number').value) + index))}</span>${state}${output}</div><button class="icon-button" type="button" data-remove-job="${job.id}" aria-label="Remove ${escapeHtml(job.file.name)}">×</button></li>`;
  }).join('');
  $<HTMLButtonElement>('#render-button').disabled = !jobs.length || rendering;
  $('#render-summary').textContent = jobs.length ? `${jobs.length} output ${jobs.length === 1 ? 'file' : 'files'} queued.` : 'Ready when your queue is.';
...
function renderReceipts(): void {
  ...
  container.innerHTML = receipts.sort((a, b) => b.renderedAt.localeCompare(a.renderedAt)).slice(0, 6).map((receipt) => `<article class="receipt"><div><strong>${escapeHtml(receipt.recipeName)} · v${receipt.recipeVersion}</strong><span>${new Date(receipt.renderedAt).toLocaleString()} · ${receipt.items.length} files</span></div><button class="text-button" type="button" data-receipt="${receipt.id}">Download JSON</button></article>`).join('');
}
...
async function importRecipe(file: File): Promise<void> {
  ...
  if (!unlocked && recipes.length >= 1 && !recipes.some((recipe) => recipe.id === parsed.id)) throw new Error('The free bench holds one saved recipe. Unlock Studio or replace your current recipe.');
  for (const key of ['intro', 'outro', 'bed'] as const) {
...
async function renderBatch(): Promise<void> {
  ...
  try {
    ...
    message('#render-message', error instanceof Error ? error.message : 'Check the recipe.', true);
    return;
  }
  rendering = true;
  clearBatchDownload();
  ...
  for (let index = 0; index < jobs.length; index += 1) {
    ...
    if (!job) continue;
    $('#progress-label').textContent = `Wrapping ${index + 1} of ${jobs.length}: ${job.file.name}`;
    try {
      ...
      job.error = error instanceof Error ? error.message : 'This file could not be rendered.';
    }
  ...
  if (successful.length) {
    const receipt: Receipt = {
      ...
      targetLufs: current.targetLufs, codec: 'WAV PCM 16-bit', measurement: 'RMS-based LUFS estimate, ±12 dB gain cap; sample-peak ceiling −0.18 dBFS',
    ...
    const container = $<HTMLElement>('#batch-download');
    container.innerHTML = `<a class="button primary" href="${batchUrl}" download="${sanitizeFilename(current.name)}-wrapped.zip">Download batch ZIP</a><span>${successful.length} WAV ${successful.length === 1 ? 'file' : 'files'} + receipt</span>`;
    container.hidden = false;
    message('#render-message', `${successful.length} of ${jobs.length} tracks wrapped. Review them above or download the batch.`);
...
function bindEvents(): void {
  ...
  $('#saved-recipes').addEventListener('change', (event) => {
    ...
    fillRecipeForm();
    message('#recipe-message', id ? `Loaded “${current.name}”.` : 'Fresh recipe ready.');
    renderQueue();
  });
  $('#new-recipe').addEventListener('click', () => { current = freshRecipe(); renderRecipeOptions(); fillRecipeForm(); message('#recipe-message', 'Fresh recipe ready.'); renderQueue(); });
  $('#recipe-form').addEventListener('submit', async (event) => {
    ...
    } catch (error) { message('#recipe-message', error instanceof Error ? error.message : 'Could not save the recipe.', true); }
  });
  $('#delete-recipe').addEventListener('click', async () => {
    if (!confirm(`Delete “${current.name}” and its saved wrapper audio from this device?`)) return;
  ...
  $('#export-recipe').addEventListener('click', async () => {
    ...
    catch (error) { message('#recipe-message', error instanceof Error ? error.message : 'Could not export the recipe.', true); }
  });
